...
static void PopulateWithBSPModel( bspModel_t *model, m4x4_t transform ){
	...
	for ( i = 0; i < model->numBSPSurfaces; i++ )
	{
		...
		/* patchshadows? */
		if ( (ds->surfaceType == MST_PATCH||ds->surfaceType == MST_PATCHFIXED) && patchShadows == qfalse ) {
			continue;
		}
		...
		case MST_PATCHFIXED:
			...
			{
				srcMesh.width = ds->patchWidth&0xffff;
				srcMesh.height = ds->patchHeight&0xffff;
				srcMesh.subdiv_x = ds->patchWidth>>16;
				srcMesh.subdiv_y = ds->patchHeight>>16;
			}
			else
			{
