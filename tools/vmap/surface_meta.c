...
static int AddMetaTriangleToSurface( mapDrawSurface_t *ds, metaTriangle_t *tri, qboolean testAdd ){
	...
	if ( ds->shaderInfo != tri->si || ds->fogNum != tri->fogNum || ds->cubemapNum != tri->cubemapNum || ds->sampleSize != tri->sampleSize ) {
		return 0;
	}
	#if 0
...
static int CompareMetaTriangles( const void *a, const void *b ){
	...
	/* then cubemaps */
	else if ( ( (const metaTriangle_t*) a )->cubemapNum < ( (const metaTriangle_t*) b )->cubemapNum ) {
		return 1;
	}
	else if ( ( (const metaTriangle_t*) a )->cubemapNum > ( (const metaTriangle_t*) b )->cubemapNum ) {
		return -1;
	...
	for ( i = 0, j = 0; i < numMetaTriangles; i = j )
	{
		...
		if ( j <= i ) {
			for ( j = i + 1; j < numMetaTriangles; j++ )
			{
				...
				if ( head->si != end->si || head->fogNum != end->fogNum || head->cubemapNum != end->cubemapNum ) {
					break;
				}
			}
		}
